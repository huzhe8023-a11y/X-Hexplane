...
def reconstruction(cfg):
    if cfg.data.datasampler_type == "rays":
        ...
        train_dataset = get_train_dataset(cfg, is_stack=True)
    print(f'train_dataset.near_far: {train_dataset.near_far}, train_dataset.scene_bbox: {train_dataset.scene_bbox}')
    #np.save('/gpfs/offline1/staff/tomograms/users/zhehu/Hexplane/ray_o/train_time2.npy',train_dataset.all_times)
    #d = train_dataset.all_imgs
    #d = d.reshape(-1,256,256)
    #np.save('/gpfs/offline1/staff/tomograms/users/zhehu/Hexplane/ray_o/train_times.npy',d)    
    test_dataset = get_test_dataset(cfg, is_stack=True)
    ...
    loss_file = os.path.join(f'{logfolder}','loss.txt')
    with open(cfg_file, "w") as f:
        OmegaConf.save(config=cfg, f=f)

    # init model.
    aabb = train_dataset.scene_bbox.to(device)
    #aabb = torch.FloatTensor([[-1,-1,-1],[1,1,1]]).to(device)
    HexPlane, reso_cur = init_model(cfg, aabb, near_far, device)
    print(f'HexPlane.near_far: {HexPlane.near_far}')
    # init trainer.
    trainer = Trainer(
        HexPlane,
        cfg,
