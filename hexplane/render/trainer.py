...
class SimpleSampler:
    ...
    def nextids(self):
        ...
        if self.curr + self.batch > self.total:
            ...
            #self.ids = torch.LongTensor(np.linspace(0,self.total-1,self.total))
            self.curr = 0
...
class Trainer:
    ...
    def sample_data(self, train_dataset, iteration):
        ...
        if self.cfg.data.datasampler_type == "rays":
            ...
            rays_train, img_train, frame_time = (
                data["rays"],
        ...
        # sample images: randomly pick one image from the training dataset and sample a batch of rays from all the rays of the image.
            ...
            rays_train, img_train, frame_time = (
                data["rays"],
                data["imgs"].to(self.device).view(-1, 1),
                data["time"],
            )
            ...
            img_train = img_train[select_inds]
        ...
        # hierarchical sampling from dyNeRF: hierachical sampling involves three stages of samplings.
            ...
            if iteration <= self.cfg.data.stage_1_iteration:
                cam_i = np.random.choice(train_dataset.all_imgs.shape[0])
                index_i = np.random.choice(
                    train_dataset.all_imgs.shape[1] // self.cfg.data.key_f_num
                )
                img_train = (
                    train_dataset.all_imgs[cam_i, index_i * self.cfg.data.key_f_num]
                    .view(-1, 3)
                    .to(self.device)
                )
                ...
                probability = GM_Resi(
                    img_train, self.global_mean[cam_i], self.cfg.data.stage_1_gamma
                )
                ...
                cam_i = np.random.choice(train_dataset.all_imgs.shape[0])
                index_i = np.random.choice(train_dataset.all_imgs.shape[1])
                img_train = (
                    train_dataset.all_imgs[cam_i, index_i].view(-1, 3).to(self.device)
                )
                rays_train = train_dataset.all_rays[cam_i].view(-1, 6)
                frame_time = train_dataset.all_times[cam_i, index_i]
                probability = GM_Resi(
                    img_train, self.global_mean[cam_i], self.cfg.data.stage_2_gamma
                )
                select_inds = torch.multinomial(
                    probability, self.cfg.optim.batch_size
                ).to(rays_train.device)
                ...
                cam_i = np.random.choice(train_dataset.all_imgs.shape[0])
                N_time = train_dataset.all_imgs.shape[1]
                ...
                img_train = img_train[select_inds]
                frame_time = torch.ones_like(rays_train[:, 0:1]) * frame_time
        return rays_train, img_train, frame_time, train_depth
    ...
    def init_sampler(self, train_dataset):
        ...
        if self.cfg.data.datasampler_type == "rays":
            ...
            self.global_mean = train_dataset.global_mean_img.to(self.device)
    ...
    def train(self):
        ...
        for iteration in pbar:
            # Sample dat
            rays_train, img_train, frame_time, depth = self.sample_data(
                train_dataset, iteration
            )
            # Render the img values of rays
            atten_map, _ = renderer(
                rays_train,
                frame_time,
                model,
            ...
            #atten_map = threshold_image(atten_map, 1.5)
            #print(f'atten_map.shape:{atten_map.shape}, img_train.shape:{img_train.shape}')
            # Calculate the loss
            loss = torch.mean((atten_map[...,None] - img_train) ** 2)*1
            #loss = torch.nn.functional.l1_loss(atten_map, img_train)
            total_loss = loss
            ...
            if iteration % self.cfg.systems.progress_refresh_rate == 0:
                pbar.set_description(
                    ...
                    #+ f" train_psnr = {float(np.mean(PSNRs)):.6f}"
                    + f" test_psnr = {float(np.mean(PSNRs_test)):.6f}"
            ...
            if (
                ...
                PSNRs_test, LOSS_test = evaluation(
                    test_dataset,
                    model,
                    self.cfg,
            ...
            if iteration in self.cfg.model.upsample_list:
                if self.cfg.model.upsampling_type == "aligned":
                    ...
                    reso_cur = N_to_reso(
                        N_voxel, model.aabb, self.cfg.model.nonsquare_voxel, self.cfg.model.aspect_ratio,
                    )
