...
def meshgrid_xy(
    ...
    """
    tensor1 = np.linspace(0,w-1,w)
    tensor2 = np.linspace(0,h-1,h)
    ii, jj = np.meshgrid(tensor1, tensor2)
    ii = torch.FloatTensor(ii)
    jj = torch.FloatTensor(jj)
    #return ii.transpose(-1, -2), jj.transpose(-1, -2)
    return ii, jj

def get_rays(w, h, c2w):
    """
    ...
    grid = torch.stack(
        ...
        dim=0)
    grid = grid.reshape([3, -1]) # 3* (w * h)
    # Rotate ray directions from camera coordinate to the world coordinate
    ray_d = torch.matmul(c2w[:3, :3], torch.tensor([0, 0, -1]).to(c2w)) # 3
    ray_o = (torch.matmul(c2w[:3, :3], grid) + c2w[:3, 3:]) # 3,(w*h)
    ray_o = ray_o.permute(1,0)
    #print(f'ray_o:{ray_o[::3000]}')
    ray_d = ray_d[None,:].expand(ray_o.shape) 
    assert ray_d.shape==(w*h, 3)
...
def data_preproc(images):
    ...
    #     -1, *images.shape[2:]
    # )  # (21, 127, 2, 1, 128, 128) -> (21*127, 2, 1, 128, 128)
...
def new_reg(images):
    ...
    images = (1-images)*256
    #images = np.where(images==0,1e-8,images)
    #images = - np.log(images)+1e-8 # take log
    #images_mean = images.mean()
    #images_std = images.std()
    #images = (images - images_mean) / images_std
    return images
...
def touint8(images):
    ...
    return (images*255).astype("float32")
